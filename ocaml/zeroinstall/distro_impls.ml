...
module Mac = struct
  ...
  let homebrew_installed = 
      let status = Sys.command "brew --version &> /dev/null" in
      status == 0
...
let get_host_distribution config : Distro.distribution =
  ...
  | "Unix" ->
      ...
      if is_debian then
        ...
        (* TODO this isn't working. correct? *)
        Mac.darwin_distribution config
      else if Mac.homebrew_installed then
        Mac.homebrew_distribution config
      else
